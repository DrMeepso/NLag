...
func _on_Lobby_Joined(lobby_id, _perms, _locked, response):
	if response == 1:
		...
		PlayerData.player_saved_position = Vector3(0,0,0)
...
func _get_lobby_members(chat = false):
	...
	if chat:
		var lobbySize = LOBBY_MEMBERS.size()
		if lobbySize > 1: 
			_update_chat( str( lobbySize + -1 ) + " other player(s) found." )
...
func _create_replication_data(id):
	...
	for actor in OWNED_ACTORS:
		if not callv("is_instance_valid", [actor]): continue
...
func _replicate_actors(list, from):
	...
	for actor in list:
		...
		var dict = {"actor_type": actor["type"], "at": Vector3(0,0,0), "rot": Vector3(0,0,0), "zone": "", "zone_owner": - 1, "actor_id": actor["id"], "creator_id": actor["owner"], "data": {}}
		emit_signal("_instance_actor", dict, from)

var Vec0 = Vector3(0,0,0)

func _sync_create_actor(actor_type, at, zone, id = - 1, creator = STEAM_ID, rotation = Vec0, zone_owner = - 1):
	randomize()
	if id == - 1: id = randi()
...
func _read_P2P_Packet(PACKET):
	...
	if PACKET_SIZE > 0:
		...
		var DATA = bytes2var(PACKET.data.decompress_dynamic( - 1, COMPRESSION_TYPE))
		var type = DATA["type"]
		...
		match type:
			...
			"message":
				...
				var user_id = PACKET_SENDER
				var user_color = DATA["color"].left(12).replace("[", "")
...
func _validate_packet_information(data, information_keys, information_types):
	var valid = true
